...
const VisitorsList: React.FC = () => {
  ...
  const getStatusBadge = (status: StatusKanban) => {
    switch (status) {
      case StatusKanban.NOVO: return <span className="px-2 py-1 bg-primary/15 text-primary rounded text-xs font-bold uppercase">Novo</span>;
      ...
      <div className="bg-gradient-to-r from-primary to-indigo-600 rounded-xl p-6 md:p-8 text-white shadow-xl flex flex-col md:flex-row items-center justify-between gap-4 md:gap-6 transform hover:scale-[1.01] transition-transform duration-300 border-2 border-white/20">
        <div className="flex items-center gap-4 md:gap-6 w-full md:w-auto">
          ...
          <div>
            ...
            <p className="text-primary-foreground/80 text-sm md:text-lg opacity-90 leading-snug">Cadastre visitantes para iniciar o acompanhamento.</p>
          </div>
        </div>
        <button
