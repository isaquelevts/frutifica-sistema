...
import { Organization, TargetAudience } from '../../shared/types/types';
import {
...
const LeaderRegister: React.FC = () => {
  ...
  const onStep2Submit = async (data: LeaderStep2FormData) => {
    ...
    try {
      ...
      const { user } = await apiFetch<{ user: any }>('/api/auth/register-leader', {
        method: 'POST',
        body: JSON.stringify({ orgId, name: step1Data.name, email: step1Data.email, password: step1Data.password }),
      });

      // 2. Criar a célula
      const cell = await apiFetch<any>('/api/cells', {
        method: 'POST',
        body: JSON.stringify({
          name: data.cellName,
      ...
      // 3. Vincular célula ao líder
      await apiFetch(`/api/users/${user.id}`, {
        method: 'PUT',
        body: JSON.stringify({ name: user.name, email: user.email, roles: user.roles, cellId: cell.id }),
      });
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <select
                      ...
                      className={`w-full px-4 py-2 rounded-lg bg-white border ${step2Form.formState.errors.dayOfWeek ? 'border-red-500' : 'border-slate-300'} text-slate-800 focus:ring-2 focus:ring-blue-500 outline-none`}
                    >
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className={`w-full px-4 py-2 rounded-lg bg-white border ${step2Form.formState.errors.time ? 'border-red-500' : 'border-slate-300'} text-slate-800 focus:ring-2 focus:ring-blue-500 outline-none`}
                    />
